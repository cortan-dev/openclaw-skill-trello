...
def run() -> None:
    ...
    parser.add_argument("--source-board", "--board", dest="source_board", help="Board name or ID for card lookup")
    parser.add_argument("--source-list", help="List name or ID for card lookup")
    parser.add_argument("--target-list", "--to-list", dest="target_list", required=True, help="Target list name or Trello list ID")
    ...
    target_board = args.target_board or args.source_board
    target = client.resolve_list(args.target_list, target_board)
